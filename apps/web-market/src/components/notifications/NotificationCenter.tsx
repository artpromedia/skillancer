...
export interface NotificationCenterProps {
  ...
  readonly onAction?: (notificationId: string, action: string) => void;
  /** Additional CSS classes */
  readonly className?: string;
  /** Max notifications to show in dropdown */
  readonly maxDropdownItems?: number;
}
...
export function NotificationCenter({
  ...
  const handleAction = useCallback(
    (notificationId: string, action: string, e: React.MouseEvent) => {
      e.stopPropagation();
      e.preventDefault();
    ...
    <div ref={dropdownRef} className={cn('relative', className)}>
        ...
        {unreadCount > 0 && (
          <span
            className={cn(
              ...
              'h-[18px] min-w-[18px] px-1 text-xs font-bold',
      ...
      {isOpen && (
        ...
        >
              ...
              {unreadCount > 0 && onMarkAllRead && (
                <button
                  className={cn(
                    ...
                    'rounded-md hover:bg-blue-50 dark:hover:bg-blue-900/30',
                    'transition-colors'
                  )}
          ...
          {/* Notification List */}
          <div className="max-h-[400px] overflow-y-auto">{renderNotificationList()}</div>
...
interface NotificationItemProps {
  ...
  readonly onMarkRead: (id: string, e?: React.MouseEvent) => void;
  readonly onDelete: (id: string, e: React.MouseEvent) => void;
  readonly onAction: (notificationId: string, action: string, e: React.MouseEvent) => void;
            ...
            <div className="mt-2 flex gap-2">
              {notification.actionButtons.map((button) => (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onAction(notification.id, button.action, e);
                  }}
                >
                  {button.title}
                </button>
...
export function useNotifications(options: UseNotificationsOptions = {}) {
  ...
  const markAsRead = useCallback(
    async (id: string) => {
      try {
        const response = await fetch(`${apiUrl}/${id}/read`, {
          method: 'POST',
        });
        if (response.ok) {
  ...
  // Add real-time notification from push
