...
export async function teamReunionRoutes(fastify: FastifyInstance) {
  ...
  // Invite member
  fastify.post('/team-reunions/:id/invite', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  fastify.post('/team-reunions/:id/respond', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
      ...
      const membership = await teamReunionService.respondToInvitation(id, userId, accept, message);
  ...
  fastify.post('/team-reunions/:id/confirm', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  // Activate reunion
  fastify.post('/team-reunions/:id/activate', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  // Complete reunion
  fastify.post('/team-reunions/:id/complete', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  fastify.get('/team-reunions/invitations/pending', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  fastify.get('/team-reunions/suggest-colleagues', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
  ...
  // Cancel reunion
  fastify.post('/team-reunions/:id/cancel', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userId = (request as any).user?.id;
      if (!userId) {
        return reply.status(401).send({ error: 'Unauthorized' });
