// @ts-nocheck
/**
 * @module @skillancer/market-svc/plugins/rate-limit
 * Rate limiting plugin for marketplace service
 *
...
import {
  createMarketRateLimitHooks,
  MarketRateLimitExceededError,
  type MarketRateLimitHooks,
...
declare module 'fastify' {
  interface FastifyInstance {
    /**
     * Rate limit hooks for marketplace endpoints
     */
    marketRateLimit: MarketRateLimitHooks;
    ...
    checkMarketRateLimit: (
      ...
      endpoint: keyof MarketRateLimitHooks
    ) => Promise<{ allowed: boolean; remaining: number; resetAt: Date }>;

    /**
     * Get rate limit info for a user across all endpoints
     */
    getMarketRateLimitStatus: (
      userId: string
    ) => Promise<Record<keyof MarketRateLimitHooks, { remaining: number; resetAt: Date }>>;
  }
}
...
async function rateLimitPluginImpl(
  ...
  if (options.redis) {
    redis = options.redis;
    app.log.info('[MarketRateLimit] Using Redis for distributed rate limiting');
  } else {
    app.log.info('[MarketRateLimit] Using in-memory rate limiting (development mode)');
    redis = createInMemoryStore();
  }

  // Create rate limiter instance with market-specific prefix
  const limiter = new RateLimiter(redis, 'market:ratelimit');

  // Create all rate limit hooks
  const hooks = createMarketRateLimitHooks(limiter);

  // Decorate fastify with rate limit hooks
  app.decorate('marketRateLimit', hooks);

  // Add helper to check rate limit without consuming
  app.decorate(
    'checkMarketRateLimit',
    async (request: FastifyRequest, endpoint: keyof MarketRateLimitHooks) => {
      const {
        MarketRateLimitConfigs,
        getRateLimitKey,
        getUserTier,
      ...
      const config = getAdjustedConfig(
        MarketRateLimitConfigs[endpoint],
        tier,
        verificationStatus,
        accountAge
      );
  ...
  app.decorate('getMarketRateLimitStatus', async (userId: string) => {
    const { MarketRateLimitConfigs, UserTierMultipliers, AccountAgeMultipliers } =
      await import('../middleware/rate-limit.js');

    // Default to PROFESSIONAL tier and ESTABLISHED account
    const tier = 'PROFESSIONAL';
    const accountAge = 'ESTABLISHED';
    const multiplier = (UserTierMultipliers[tier] || 1) * (AccountAgeMultipliers[accountAge] || 1);

    const status: Record<string, { remaining: number; resetAt: Date }> = {};

    for (const [endpoint, baseConfig] of Object.entries(MarketRateLimitConfigs)) {
      const key = `market:${endpoint}:user:${userId}`;
      const config = {
    ...
    return status as Record<keyof MarketRateLimitHooks, { remaining: number; resetAt: Date }>;
  });

  // Global error handler for rate limit errors
  app.addHook('onError', async (_request, reply, error) => {
    if (error instanceof MarketRateLimitExceededError) {
  ...
  app.get('/rate-limit-status', async (request, reply) => {
    ...
    const status = await app.getMarketRateLimitStatus(user.id);
    return {
      userId: user.id,
      limits: status,
