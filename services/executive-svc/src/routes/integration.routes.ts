...
export async function integrationRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/engagements/:engagementId/integrations', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post('/engagements/:engagementId/integrations', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { engagementId } = request.params as { engagementId: string };
  ...
  fastify.delete('/integrations/:integrationId', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post('/integrations/:integrationId/refresh', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { integrationId } = request.params as { integrationId: string };
  ...
  // Update sync status
  fastify.patch('/integrations/:integrationId/sync', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { integrationId } = request.params as { integrationId: string };
  ...
  fastify.post('/integrations/:integrationId/sync', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { integrationId } = request.params as { integrationId: string };
  ...
  fastify.get('/integrations/:integrationId/data', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { integrationId } = request.params as { integrationId: string };
  ...
  // Toggle sync enabled
  fastify.patch('/integrations/:integrationId/toggle-sync', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const { integrationId } = request.params as { integrationId: string };
  ...
  fastify.get('/profiles/:executiveProfileId/tool-configs', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  // Update tool config
  fastify.put('/profiles/:executiveProfileId/tool-configs/:toolKey', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
