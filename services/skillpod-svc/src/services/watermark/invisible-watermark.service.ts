...
export function createInvisibleWatermarkService(): InvisibleWatermarkService {
  ...
  function extractLSB(
    ...
    // First, try to find magic bytes
    const bytesPerPixel = 4;
    const _totalPixels = Math.floor(imageData.length / bytesPerPixel);
    const channels = config.channels || ['B'];
    const strength = config.strengthValue || 1;
    const redundancy = config.redundancy || 1;

    // Extract header bits (7 bytes = 56 bits)
    ...
    for (let i = 0; i < headerPositions.length && headerBits.length < 56; i++) {
      ...
      if (redundancy > 1) {
        ...
        if ((i + 1) % redundancy === 0) {
          const startIdx = i - redundancy + 1;
          let ones = 0;
          for (let j = 0; j < redundancy; j++) {
            const p = headerPositions[startIdx + j];
            ones += (imageData[p] >> (strength - 1)) & 1;
          }
          headerBits.push(ones > redundancy / 2 ? 1 : 0);
        }
      } else {
        headerBits.push(bit);
      }
    ...
    const headerData = bitsToData(headerBits);
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (redundancy > 1) {
        if ((i + 1) % redundancy === 0) {
          ...
          // Detect errors when not unanimous
          if (ones !== 0 && ones !== redundancy) {
            errors++;
          }
          allBits.push(ones > redundancy / 2 ? 1 : 0);
        }
      } else {
        allBits.push(bit);
      }
    ...
    const errorRate = errors / (allBits.length / redundancy);
